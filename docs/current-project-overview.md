...
- 首页顶部关系区不展示 App 图标或未确定 App 名称，应展示左侧“饲养员”头像位、中间爱心、右侧“吃货”头像位；未绑定/未选择时头像位是空心加号。
- 首页只应该保留一套当前情侣首页 Tab/内容结构，不允许同时出现旧首页 Tab 和新首页 Tab 两套入口或标签。
- 角色选择不是普通跳转按钮：选择“饲养员/吃货”后应保存当前用户身份，并把当前用户放到对应头像位；另一侧保留伴侣空位或伴侣头像。选择身份后不应直接跳到点菜页，除非已经完成身份保存并由明确按钮触发点菜。
...
- 伴侣账号/用户管理：围绕“邀请 TA / 绑定伴侣 / 解除绑定 / 显示双方身份”落地，不做传统后台用户管理。
...
- 饲养员在左、吃货在右是固定关系位；用户选择饲养员就显示在左侧，选择吃货就显示在右侧。
