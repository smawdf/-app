...
import com.myorderapp.data.remote.recipe.BingDishImageSource
...
val networkModule = module {
    ...
    single { BingDishImageSource(get()) }
    single { XiachufangRecipeSearchSource(get()) }
    single<ExternalDishImageSource> {
        CombinedExternalDishImageSource(
            sources = listOf(
                get<BingDishImageSource>(),
                get<SpoonacularExternalDishImageSource>(),
                get<TheMealDbExternalDishImageSource>()
            )
        )
    }
}
